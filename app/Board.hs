...
moveBlinky (Ghost name loc (Destination point, [t]) curr next d u inf) b 
  = Ghost name t (Destination  (getPlayerDestination b), nukeButSlow 1 b (getPlayerDestination b) (addPaths (getValidNeighbors b loc 1) []) [point]) curr next d u inf
...
nukeButSlow order b dest ((Neighbor (Destination pt) tracksToNeighbor, path):queue) visited 
  | pt == dest = path ++ (genTracks 0.5 (getDir pt (head tracksToNeighbor)) (head tracksToNeighbor) pt)
  | otherwise = 
    if pt `elem` visited 
  ...
  where
    ...
    nextsWithPaths = addPaths next (path ++ (genTracks 0.5 (getDir pt (head tracksToNeighbor)) (head tracksToNeighbor) pt))
    getDir (x1, y1) (x2, y2) 
